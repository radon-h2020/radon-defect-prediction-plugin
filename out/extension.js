...
function getWebviewContent(data) {
    ...
    Object.entries(data.metrics).forEach(([key, value]) => {
        ...
        if (key in thresholds) {
            if (value <= thresholds[key]['median'])
                color_class = 'white';
            else if (value <= thresholds[key]['q3'])
                color_class = 'yellow';
            else if (value <= thresholds[key]['outlier'])
                color_class = 'orange';
            else
                color_class = 'red';
            if (key == 'num_unique_names') {
                ...
                if (value >= thresholds[key]['median'])
                    color_class = 'white';
                else if (value >= thresholds[key]['q1'])
                    color_class = 'yellow';
                else if (value >= thresholds[key]['q1'] - 1.5 * iqr)
                    color_class = 'orange';
                else
                    color_class = 'red';
            }
